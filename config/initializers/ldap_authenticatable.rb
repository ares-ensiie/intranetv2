        ...
        if params[:user]
          ...
          if results.length == 1
            ...
            if ldap.bind
              ...
              user.update({uid: params[:user][:email], password: "dummydummy", promo: results[0][:roomnumber][0], lastname: results[0][:sn][0], name: results[0][:givenname][0], phone: results[0][:mobile][0], address: results[0][:postalAddress][0]})
              user.save!
              success!(user)
            else
              fail(:invalid_login)
